...
public class SpecialView extends View {
	...
	public SpecialView(Type type, boolean recieving) {
		...
		b.setClickAction(() -> Campfire.getInstance().endSpecial(true));
		b.addButtonComponent(i);
		b.pauseHover();
		addComponent(b);
		
		Button close = new GradientButton(new Rectangle(53, 41, 7, 14), new Color(126, 0, 14), new Color(191, 0, 9));
		close.setClickAction(() -> Campfire.getInstance().endSpecial(true));
		close.addButtonComponent(new Image(new Rectangle(17.5, 17.5, 65, 65), "exit.png"));
		close.freezeShadow();
		addComponent(close);
		close.setOpacity(0);
